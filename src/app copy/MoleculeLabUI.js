...
const MoleculeLabUI = {
   ...
   createHeaderPanel() {
    ...
    // Left Title Group
    const titleGroup = this.createElement("div", { className: "molecule-lab-title-group" });
    const eyebrow = this.createElement("p", {
        className: "molecule-lab-eyebrow",
        textContent: "Molecule Lab - Chemical Bonding Zone"
    });
    
    const titleRow = this.createElement("div", { className: "molecule-lab-title-row" });
    const title = this.createElement("h1", {
        className: "molecule-lab-title",
        textContent: "Molecular Synthesis"
    });

    titleRow.appendChild(title);
    titleGroup.append(eyebrow, titleRow);

    // Right Stats Panel
    const statsGroup = this.createElement("div", { className: "molecule-lab-stats-group" });

    // 1. Library Title (Positioned above counter rows)
    const libraryTitle = this.createElement("span", {
        className: "molecule-lab-library-title",
        textContent: "LIBRARY"
    });

    // 2. Synthesized Stat Row
    const synthesizedRow = this.createElement("div", { className: "molecule-lab-stat-row" });
    const synthesizedLabel = this.createElement("span", { textContent: "Molecules Synthesized: " });
    this.synthesizedCountEl = this.createElement("strong", {
        id: "stat-synthesized-molecules",
        textContent: "0"
    });
    synthesizedRow.append(synthesizedLabel, this.synthesizedCountEl);

    // 3. Investigated Stat Row
    const investigatedRow = this.createElement("div", { className: "molecule-lab-stat-row" });
    const investigatedLabel = this.createElement("span", { textContent: "Properties Investigated: " });
    this.investigatedCountEl = this.createElement("strong", {
        id: "stat-properties-investigated",
        textContent: "0"
    });
    investigatedRow.append(investigatedLabel, this.investigatedCountEl);

    statsGroup.append(libraryTitle, synthesizedRow, investigatedRow);
    header.append(titleGroup, statsGroup);
